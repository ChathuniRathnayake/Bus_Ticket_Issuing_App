...
export default function AdminLogin({ goPassenger }) {
  ...
  const handleLogin = async () => {
    ...
    if (res.ok) alert("Admin login successful: " + data.token);
  ...
  return (
    <div className="w-96 p-6 bg-white rounded shadow">
      ...
      <button className="w-full p-2 bg-red-600 text-white rounded" onClick={handleLogin}>Login</button>
      <button className="text-sm text-blue-500 mt-3" onClick={goPassenger}>Back to Passenger Login</button>
    </div>
  );
}
